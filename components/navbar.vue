  ...
  <div>
    ...
    <nav id="navbar" class="z-40 bg-gray-300 fixed inset-x-0">
      <div class="px-6 pb-1 mx-auto">
        <div class="flex justify-between">
          <div class="flex space-x-4">
            <!-- logo -->
            <div>
              <nuxt-link to="/" class="flex items-center py-3 px-3 text-gray-700">
                <svg class="w-7 h-7 mr-1 text-green-700" xmlns="http://www.w3.org/2000/svg" fill="none"
          ...
          <!--second nav-->
          <div class="hidden md:flex items-center space-x-3" v-if="!user">
          ...
          <div v-else>
            <div class="font-semibold py-1 px-3 text-gray-700">
              {{ user?.gamerTag }}
            </div>
          </div>
      ...
      <div v-if="toggle_menu" id="mobile-menu" class="mobile-menu md:hidden">
        ...
        <nuxt-link to="/fighters" class="block py-3 px-4 hover:bg-gray-200">Fighters</nuxt-link>
      </div>
...
let user = ref(null);
if (process.client) {
  user.value = JSON.parse(sessionStorage.getItem("authenticatedGamer"))
}
